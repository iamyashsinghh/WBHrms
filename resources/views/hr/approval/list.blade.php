...
<script>
    ...
    $(document).ready(function() {
        $('#serverTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('hr.approval.ajax_list') }}",
                type: 'GET'
            },
            columns: [{
                    data: 'id',
                    ...
                    render: function(data, type, row) {
                        if(data == 0){
                            return `<div class="my-1 button-group d-flex">
                                            <a href="{{route('hr.approval.update_status')}}/${row.id}/2" class="mx-1 btn text-light btn-sm buttons-print" style="background-color: var(--wb-dark-red)">Reject</a>
                                            <a href="{{route('hr.approval.update_status')}}/${row.id}/1" class="mx-1 btn text-light btn-sm buttons-print" style="background-color: var(--wb-renosand)">Approve</a>
                                        </div>`;
                        }else if(data == 1){
                            return `<div class="my-1 button-group d-flex">
