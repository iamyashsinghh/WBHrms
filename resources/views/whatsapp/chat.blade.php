...
<div class="modal fade" id="wa_msg" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="text-sm modal-body whatsapp_msg">
            </div>
            <form action="" method="post" id="wa_msg_form">
                @csrf
                <div class="text-sm modal-body">
                ...
                <div class="text-sm modal-footer">
                    <div class="mx-5">
                    <a href="javascript:void(0);" class="m-1 btn btn-sm text-light" style="background-color: var(--wb-dark-red)" id="sendMessageBtnGreetMsg" title="Hi (Name of person in lead), I'm {{ $userName }} your Wedding Planning assistant. Would you like me to help you find venues and wedding vendors for your wedding? (Some btns)">Rm Greet Msg</a>
                    <a href="javascript:void(0);" class="m-1 btn btn-sm text-light" style="background-color: var(--wb-dark-red)" id="sendMessageBtnHi" title="Hi">Hi</a>
                    </div>
                    <a href="javascript:void(0);" class="m-1 btn btn-sm btn-secondary" data-bs-dismiss="modal">Close</a>
                    <a href="javascript:void(0);" class="m-1 btn btn-sm text-light"
...
<script>
    ...
    $btn.prop('disabled', true);
        ...
        $.ajax({
            url: '{{ ('whatsapp_chat.send.greet_btn') }}',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function(response) {
    ...
    function wamsg(num) {
        ...
        function fetchMessages(page) {
            ...
            // const data_url = `{{ 'whatsapp_chat.get', '' }}/${id}?page=${page}`;
            $.ajax({
                url: data_url,
                type: 'GET',
                dataType: 'json',
        ...
        function fetchNewMessages() {
            if(lastMessageTimestamp != ''){
                // const data_url = `{{ 'whatsapp_chat.get_new', ''}}/${id}?lastTimestamp=${lastMessageTimestamp}`;
            $.ajax({
                url: data_url,
                type: 'GET',
                dataType: 'json',
